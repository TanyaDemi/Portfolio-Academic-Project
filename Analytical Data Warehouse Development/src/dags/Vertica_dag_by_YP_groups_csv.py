...
def load_dataset_file_to_vertica(
    ...
    log.info(f"Читаем файл {dataset_path}. Количество строк: {df.shape[0]}")
    ...
    with contextlib.closing(get_vertica_conn()) as conn:
        with contextlib.closing(conn.cursor()) as cur:
            ...
            FROM STDIN DELIMITER ',' ENCLOSED BY '"' NULL AS '\\N'
            """
            for start in range(0, num_rows, chunk_size):
                end = min(start + chunk_size, num_rows)
                chunk = df.iloc[start:end]
                # Приводим все данные к строке и заменяем строковые представления пропусков
                ...
                if start == 0:
                    log.info(f"Первый чанк CSV (первые 200 символов): {csv_data[:200]}")
...
def s3_to_vertica_groups():

    start = EmptyOperator(task_id='start')
    end = EmptyOperator(task_id='end')

    @task
    def download_groups_file() -> str:
    ...
    @task
    def load_groups_to_vertica(file_path: str):
    ...
    file_path = download_groups_file()
    load_task = load_groups_to_vertica(file_path)

    start >> file_path >> load_task >> end


dag = s3_to_vertica_groups()
