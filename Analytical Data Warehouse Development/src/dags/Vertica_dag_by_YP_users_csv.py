    ...
    with contextlib.closing(get_vertica_conn()) as conn:
        with contextlib.closing(conn.cursor()) as cur:
            ...
            for start in range(0, num_rows, chunk_size):
                ...
                # Явно заменим NaN на \N (после всех преобразований)
                chunk = chunk.replace({pd.NA: '\\N', pd.NaT: '\\N', None: '\\N'})
...
def s3_to_vertica_users():

    start = EmptyOperator(task_id='start')
    end = EmptyOperator(task_id='end')

    @task
    def download_users_file() -> str:
        file_name = 'users.csv'
    ...
    def load_users_to_vertica(file_path: str):
    ...
    file_path = download_users_file()
    load_task = load_users_to_vertica(file_path)

    start >> file_path >> load_task >> end


dag = s3_to_vertica_users()
