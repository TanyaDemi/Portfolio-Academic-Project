...
SELECT
    rate_dt::DATE,
    CASE
        WHEN currency_code_to = 420 THEN currency_code_from
...
WHERE account_number IN (
    ...
    WHERE transaction_dt::DATE = '2022-10-31'
...
WHERE tr.status = 'done'
  AND tr.transaction_dt::DATE = '2022-10-31';
"""

# 4. Обновление метрик (global_metrics)
SQL_GLOBAL_METRICS = f"""
DELETE FROM {DWH}.global_metrics
WHERE date_update = '2022-10-31';
...
@dag(
    dag_id="2_datamart_update_2022-10-31",
    start_date=pendulum.datetime(2022, 10, 1, tz="UTC"),
    schedule=None,  # Ручной запуск для конкретной даты
    catchup=False,
    max_active_runs=1,
    default_args={
        "retries": 2,
    ...
    def exec_sql(sql: str):
        with vt_hook.get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute(sql)
                conn.commit()
                print(f"Executed: {sql.split()[0]} - {cur.rowcount} rows affected")

    @task
    def dim_rates():
        exec_sql(SQL_DIM_RATES)

    @task
    def accounts():
        exec_sql(SQL_ACCOUNTS)

    @task
    def fact_transactions():
        exec_sql(SQL_FACT_TRX)

    @task
    def global_metrics():
        exec_sql(SQL_GLOBAL_METRICS)
